...
- This pass completes the public-safe visible-header mapping from committed screenshot manifests, source-summary tables, and header registries.
...
- The user reported a new self-email with screenshots for the four dataset headers, temperature gradient, and equations. Those screenshots are approved review material for Drive/OSL, but the raw images are still excluded from GitHub.
...
- Confirm whether `MLK` and `ETG` appear in any approved source, workbook tab, screenshot, or `wellnametodataset` mapping.
...
- Branch: `codex/prompt15-visible-header-mapping-20260619`
