...
| Authorized runtime | `dashboard/runtime/` and `doe_anaconda_final_kit/` | Source-driven readiness, grouped-well split scaffold, three-dataset runtime, local model-run tracker, and the GitHub-facing V19 DOE/Anaconda notebook are implemented. V19 keeps row-level predictions, fitted models, and runtime logs in ignored DOE folders while its final export cell creates a compact `V19_output_review_<run_id>` folder and saved Outlook draft for review. It explicitly labels known-well transfer ranking as development optimization, adds larger heatmaps, and exports feature-family weighting sensitivity diagnostics. | Run V19 on the DOE desktop, inspect saturation/occurrence model ranking, feature-family weight heatmaps, core/stability audits, and keep stability as context unless a valid join is documented |
...
| W9 | Authorized-data execution | In progress | Three approved workbooks can be scanned/trained locally; use the V19 notebook in `doe_anaconda_final_kit/` for the next known-four-well development run and its built-in compact review-folder/Outlook export while row-level outputs stay ignored | W5, authorization | Reproducible authorized outputs exist with mentor-reviewed target authority, explicit development-vs-blind validation language, and reviewed stability/core usage |
