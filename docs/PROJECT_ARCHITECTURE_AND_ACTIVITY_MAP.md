...
| Source library | Index recovered; 2026-06-13 stability source bundle documented, uploaded locally in OpenScienceLab, connected to a Structural Explorer source panel, and paired with a committed public snapshot fallback for website rendering | In progress | Use OpenScienceLab as the heavy-data workbench, then commit only derived public-safe stability products and digitize/georeference OM-222 if no ready GIS derivative is found |
...
| W2 | Organize source intake | In progress | The public stability source bundle is documented and uploaded locally under `data/source_library/`; the committed snapshot under `data/public_stability_snapshot/` keeps the website usable while raw bundles stay out of Git | W1 | Every recovered file has a location and classification |
