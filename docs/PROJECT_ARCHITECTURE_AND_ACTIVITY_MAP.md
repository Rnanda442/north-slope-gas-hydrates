...
| Authorized runtime | `dashboard/runtime/` and `doe_anaconda_final_kit/` | Source-driven readiness, grouped-well split scaffold, three-dataset runtime, local model-run tracker, GitHub-facing V19 DOE/Anaconda notebook, and V19-style full V20 DOE/Anaconda comparison notebook are implemented. V20 lives at `doe_anaconda_final_kit/DOE_MASTER_ML_PIPELINE_EQUATION_FIRST_V20_THREE_VERSION_COMPARISON_ANN.ipynb`, compares V20A baseline-safe, V20B equation-dominance, and V20C HYDRATE-02 core-prior diagnostics, validates the required V20 packet tables before email export, and keeps row-level predictions, fitted models, and runtime logs in ignored runtime folders. | Download/run the V20 notebook on the DOE desktop with `actual_core_data_combined.xlsx` and the approved well-log workbooks; inspect the clean-summary V20 sheets, saturation/occurrence metrics, feature weights, core-prior audit, packet audit, and bin/well bias before making claims |
...
| W9 | Authorized-data execution | In progress | Three approved workbooks can be scanned/trained locally; V19 remains the current known-four-well development run reference, and V20 now adds a downloadable self-contained notebook in `doe_anaconda_final_kit/` for the three fixed V20 comparisons. V20 uses `actual_core_data_combined.xlsx` for core context / HYDRATE-02 candidate target evidence, writes runtime outputs only to ignored folders, and the final export fails before email packet creation if the required row-free V20 tables are absent or failed. | W5, authorization, approved well-log workbooks, `actual_core_data_combined.xlsx`, DOE/Anaconda sklearn environment | Reproducible authorized outputs exist with mentor-reviewed target authority, explicit development-vs-blind validation language, reviewed stability/core usage, and V20 packets that include the required clean-summary sheets plus row-free CSV/JSON evidence |
