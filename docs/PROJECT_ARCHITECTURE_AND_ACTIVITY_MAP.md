...
| Authorized runtime | `dashboard/runtime/`, `doe_jupyter_runtime_pack/` | Source-driven readiness, grouped-well split scaffold, three-dataset runtime, local model-run tracker, GitHub-downloadable DOE Jupyter transfer pack, and four-well stability spatial-join handoff implemented | Run the Jupyter pack on the DOE desktop with the real local workbooks and temperature-gradient CSV, then review row-free graph/table/map outputs before any public release |
| Well-log tests | `tests/` | Unified branch needs a fresh local test run before recording a current count; recent branch-specific historical checks include `122 passed, 2 skipped` for the project-revision/unified-map branch and `119 passed, 2 skipped` for the editable visual-match deck branch | Expand with workbook-derived unit, label, and alignment cases |
...
| W9 | Authorized-data execution | In progress | Three approved workbooks can be scanned/trained locally; cleaned multi-saturation prototype outputs can be reviewed through the local Model Run Tracker while row-level outputs stay ignored | W5, authorization | Reproducible authorized outputs exist with mentor-reviewed target authority and validation |
...
|---|---|---|
| 2026-06-19 | Added DOE ML GitHub reproduction prompt pack | Created `DOE_ML_PIPELINE_GITHUB_REPRODUCTION_PROMPTS_2026-06-19.md` after auditing current runtime scripts, graph/export code, tests, runbooks, source-ledger docs, and the public/DOE boundary. The prompt pack separates headers-only work from DOE-only real-data execution and defines the next missing code tasks: a master DOE orchestrator, a dedicated graph export CLI, a code-only transfer zip builder, and source-support matrix. |
| 2026-06-19 | Added four-well/case online core-data source index | Created a public-safe source index and CSV for Mount Elbert/MTE, Ignik Sikumi/IGS, Hydrate-01, and HYDRATE 02 core, pressure-core, NMR, lithology, grain-size, porosity, permeability, saturation, and logging sources. The index keeps Hydrate-01 and HYDRATE 02 as workbook-scope candidates until header-only DOE/OSL mapping confirms active cases, excludes MLK/ETG as unverified well aliases, and routes raw supplements/datasets to Drive/OSL only. |
| 2026-06-19 | Built editable Slides 3-7 candidate package | Executed delegated Prompts 18-20 by adding `build_editable_slides_3_7_candidate.py`, a separate V5.5 candidate PPTX, PNG previews, contact sheet, editability audit, source/claim notes, and handoffs. Slides 3-7 are no longer whole-slide raster screenshots in the candidate: labels, callouts, lanes, equation cards, captions, and legends are editable, while real-data-dependent log/core panels remain DOE export placeholders and the stability map remains context only. |
| 2026-06-19 | Added the next delegated execution prompt pack | Created `NEXT_10_DELEGATED_EXECUTION_PROMPTS_2026-06-19.md` to continue the four-chat workflow from the GitHub visibility audit. The new prompts cover DOE/Jupyter code packaging, DOE real-data runtime execution, four-well core/lithology/source searches, header-only workbook mapping, native editable deck infrastructure, editable slide rebuilds for Slides 1-9, final deck assembly, and Word companion sync while preserving public/approved-data boundaries. |
...
| 2026-06-16 | Created slide source map | Verified that the requested `C:\Users\gargi\...` source roots are not mounted on this machine, indexed the accessible repo-local references, public stability products, GIS layers, website captures, and deck versions, and added `docs/source_map_for_slides.md` so future slide edits use the final Gmail-style spine, V5.2 as support, and V5.3 only as flawed reference. |
...
| 2026-06-18 | Captured website 2D well maps and staged basemap landmark sources | Saved the current website stability-status and temperature-coverage map captures, documented recommended slide placement, and staged public DNR/AKDOT/Census/GNIS/TAPS GIS candidates in ignored `data/source_library/basemap_landmarks_2026_06_18/` so OSL can rebuild clearer North Slope maps with Prudhoe Bay, Kuparuk, Milne Point, roads, communities, and pipeline context. |
...
| 2026-06-19 | Added header-only screenshot manifest and public case-well/API map anchors | Pulled the latest self-email screenshot attachments into ignored local storage, committed only a header/source manifest, verified the screenshot workbook structure as `MTE` and `IGS` actual well sheets plus `MTE_refined` and `IGS_refined` processed sheets, and added a public Alaska well metadata index for Mount Elbert, Ignik Sikumi, Hydrate-01, HYDRATE 02, and associated public anchors. The unified website map now displays these public case-well/API labels as a separate marker group while preserving the stability-status color boundary and avoiding any row-level screenshot/workbook data in GitHub. |
| 2026-06-19 | Added DOE Jupyter ML graph transfer pack | Added `doe_jupyter_runtime_pack/` with a README, config template, Jupyter notebook, graph-output checklist, and wrapper script so the DOE desktop can download the repo, point to local curated workbooks, and generate header scans, leakage audits, multi-saturation runs, model-run review assets, slide/Word graph exports, and optional equation-derived visuals while keeping raw rows, predictions, models, and populated configs out of GitHub. |
| 2026-06-19 | Added DOE stability spatial-join handoff | Added `run_spatial_stability_join.py`, a DOE spatial/GIS checklist, notebook review cells, wrapper integration, and tests so the DOE desktop can compare the four public case-well anchors against committed stability-screen points, generate local context tables/maps, and keep stability as a post-model P-T context overlay rather than hydrate occurrence or saturation truth. |
| 2026-06-19 | Built and imported the native editable nine-slide deck rebuild | Created `docs/project_blueprints/EDITABLE_REBUILD_North_Slope_Gas_Hydrate_ML_Workflow_2026-06-19.pptx` with PowerPoint-native titles, text boxes, cards, arrows, callouts, circles, dividers, equations, and status panels instead of full-slide screenshots. Rendered all nine slides, generated `editable_rebuild_contact_sheet_2026_06_19.png`, wrote a shape audit confirming no slide is only one full-slide image, rendered the Slide 2 P-T diagram from the recovered CSV, imported a new native Google Slides copy at <https://docs.google.com/presentation/d/1ts1mI95SqQibox5xgkNbH6RdnJ3d5lwosdojr26a4IA>, saved Drive-rendered thumbnails/contact sheet for import QA, and kept all results language guarded with no hydrate-proof, occurrence, saturation, trained-metric, or sweet-spot claims. |
| 2026-06-19 | Corrected the editable deck into a V5.5 visual-match rebuild | User feedback said the first editable rebuild did not look enough like the previous V5.5 review deck. Recovered the exact prior Drive review panels as source/reference assets, rebuilt a corrected visual-match PPTX at `docs/project_blueprints/EDITABLE_VISUAL_MATCH_North_Slope_Gas_Hydrate_ML_Workflow_2026-06-19.pptx`, preserved the complex workflow/runtime visual plates and previous Slide 1 visual content for fidelity, rebuilt Slide 5 equation cards as native editable objects, verified no slide is a full-slide image, imported the corrected deck as native Google Slides at <https://docs.google.com/presentation/d/1cmGnsgcgkAcdP2-2ozCE2uyKb4YE0JuQoQYItxGWkHA>, and saved local/Drive contact sheets plus shape audit under `docs/project_blueprints/presentation_assets/editable_visual_match_2026_06_19/`. |
| 2026-06-19 | Cleaned the visual-match editable deck after user review | Rebuilt Slide 4 as a native editable audience workflow, rebuilt Slide 7 as the unified North Slope well + stability context map with editable callouts/status/caveat, replaced the marked-up Slide 2 hydrate-structure raster with an editable no-circle schematic, cleaned Slide 5 equation labels/fraction layout, regenerated the PPTX/renders/audit, imported cleanup v2 as native Google Slides at <https://docs.google.com/presentation/d/14l8zzQ8mbxf-adWIMphd1jorepqY3hqKdbGnn-vgb3I>, and saved fresh Drive-rendered thumbnails for Slides 2, 4, 5, and 7 under `docs/project_blueprints/presentation_assets/editable_visual_match_2026_06_19/drive_import_thumbnails_cleanup_v2/`. |
