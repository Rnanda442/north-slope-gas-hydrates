...
| W9 | Authorized-data execution | In progress | Three approved workbooks can be scanned/trained locally; V19 remains the current known-four-well development run reference, and V20 now adds a downloadable self-contained notebook in `doe_anaconda_final_kit/` for the three fixed V20 comparisons. V20 uses `actual_core_data_combined.xlsx` for core context / HYDRATE-02 candidate target evidence and writes runtime outputs only to ignored folders. | W5, authorization, approved well-log workbooks, `actual_core_data_combined.xlsx`, DOE/Anaconda sklearn environment | Reproducible authorized outputs exist with mentor-reviewed target authority, explicit development-vs-blind validation language, and reviewed stability/core usage |
