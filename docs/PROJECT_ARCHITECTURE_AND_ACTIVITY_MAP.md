...
| Authorized runtime | `dashboard/runtime/` and `doe_anaconda_final_kit/` | Source-driven readiness, grouped-well split scaffold, three-dataset runtime, local model-run tracker, and the GitHub-facing V16 final-focus DOE/Anaconda notebook are implemented. V16 keeps row-level predictions, fitted models, and runtime logs in ignored DOE folders while its final export cell now creates a compact `V16_output_review_<run_id>` folder and saved Outlook draft for review. | Run V16 on the DOE desktop, inspect the compact core/stability/model-selection/WLC review artifacts, and keep stability as context unless a valid join is documented |
...
| W9 | Authorized-data execution | In progress | Three approved workbooks can be scanned/trained locally; use the V16 notebook in `doe_anaconda_final_kit/` for the next final-focus ANN run and its built-in compact review-folder/Outlook export while row-level outputs stay ignored | W5, authorization | Reproducible authorized outputs exist with mentor-reviewed target authority and validation |
