...
| Authorized runtime | `dashboard/runtime/` and `doe_anaconda_final_kit/` | Source-driven readiness, grouped-well split scaffold, three-dataset runtime, local model-run tracker, GitHub-facing V19 DOE/Anaconda notebook, and self-contained V20 DOE/Anaconda comparison notebook are implemented. V20 lives at `doe_anaconda_final_kit/DOE_MASTER_ML_PIPELINE_EQUATION_FIRST_V20_THREE_VERSION_COMPARISON.ipynb`, compares V20A baseline-safe, V20B equation-dominance, and V20C HYDRATE-02 core-prior diagnostics, and keeps row-level predictions, fitted models, and runtime logs in ignored runtime folders. | Download/run the V20 notebook on the DOE desktop with `actual_core_data_combined.xlsx` and the private standardized model matrix; inspect saturation/occurrence metrics, feature weights, core-prior audit, and bin/well bias before making claims |
...
| W9 | Authorized-data execution | In progress | Three approved workbooks can be scanned/trained locally; V19 remains the current known-four-well development run reference, and V20 now adds a downloadable self-contained notebook in `doe_anaconda_final_kit/` for the three fixed V20 comparisons. V20 uses `actual_core_data_combined.xlsx` for core context / HYDRATE-02 candidate target evidence and writes runtime outputs only to ignored folders. | W5, authorization, private standardized model matrix, DOE/Anaconda sklearn environment | Reproducible authorized outputs exist with mentor-reviewed target authority, explicit development-vs-blind validation language, and reviewed stability/core usage |
