...
| Authorized runtime | `dashboard/runtime/` | Source-driven readiness, grouped-well split scaffold, three-dataset runtime, and local model-run tracker implemented | Complete workbook-derived input mapping, stability-context joins, and validation-ready model evaluation |
...
| Presentation | Current local baseline is the V5.5 Slide 3 signal-response update deck generated from `docs/project_blueprints/build_full_workflow_diagram_deliverables.py`: `docs/project_blueprints/V5_5_SLIDE3_SIGNAL_RESPONSE_UPDATE_North_Slope_Gas_Hydrate_ML_Workflow_Slides_2026-06-17.pptx`. It preserves the original personal opener, nine-slide spine, full complex workflow on slide 4, equations/unit gates on slide 6, and complex ML runtime architecture on slide 7. Slide 2 remains source-backed with selected USGS/DOE stability visuals, the project website regional map, hydrate structure/type language, methane/Structure I baseline framing, and the stability-not-proof guardrail. Slide 3 now uses the local source-backed signal-response package to show cleaned depth-aligned stability context, clean sand (GR/gamma), pore space (phi/RHOB), resistivity (Rt), P-wave speed (Vp), rigidity (Vs/mu-rho), fluid/core checks (NMR/core), hydrate-compatible clean sand, a resistive mimic interval, and Y-only target exclusions. Caliper/washout/bad-hole QC is treated as upstream preprocessing for this slide, not a visible interpretation track. The verified Drive copies are native Google Slides/Docs: `https://docs.google.com/presentation/d/1MuMBhO_IQ0sXGCr5IKcdLRUlEm_CHKsrHWSHCzB3DCA` and `https://docs.google.com/document/d/180KwTmIQlkQr2Zbu5MUt78oL1nj9Zh_y-3KahtQbpMw`. | Ready for mentor review | Use the V5.5 Slide 3 signal-response update package for mentor review; preserve V5.4 and earlier V5.5 as provenance/reference |
...
V5.5 Slide 3 signal-response update Word/PPT rebuild is now the working local
...
Slide 3 signal-response update package. Use the current V5.5 diagram assets for that
purpose: the rebuilt source-backed context panel, signal-response stack, full
...
done/not-claimed/next panel, and V5.5 Slide 3 signal-response contact sheet. The
V5.5 Slide 3 signal-response update PPTX/Word companion are the active
...
| 2026-06-14 | Added selected-well temperature/phase audit plot | Added a Structural Explorer plot for a selected well showing the methane 5 ppt phase boundary, sampled G10015 profile points, permafrost/TVD references, and screen top/base markers where available, clearly labeled as an audit plot rather than the full raw G10015 profile |
...
| 2026-06-17 | Prepared Slide 3 signal-response source package | Added `docs/project_blueprints/presentation_assets/slide_03_signal_response_sources_2026_06_17/` with a public-safe synthetic depth-stack template, CSV-derived methane 5 ppt stability inset, USGS public-domain log/QC crops, project regional context visual, README, source-to-visual matrix, and a build-ready Slide 3 outline. The package keeps publisher/Drive-only ANS figures as citation/source-badge support rather than committed image crops and sets the next action as rebuilding Slide 3 from co-moving signal logic, not as another parameter-range table. |
| 2026-06-17 | Refined V5.5 Slide 3 after mentor QC clarification | Regenerated `V5_5_SLIDE3_SIGNAL_RESPONSE_UPDATE_North_Slope_Gas_Hydrate_ML_Workflow_Slides_2026-06-17.pptx`, `V5_5_SLIDE3_SIGNAL_RESPONSE_UPDATE_North_Slope_Gas_Hydrate_ML_Workflow_Companion_2026-06-17.docx`, Slide 3 PNG, and contact sheet; removed the visible caliper/washout/bad-hole track because those checks are already handled upstream; relabeled the stack with beginner-readable signal names plus mnemonics; added project 2D stability map context, NMR/core definitions, stiffness/geomechanics support, 100 percent methane and 5 ppt salinity baseline language, and retained stability-as-context/Y-only guardrails. |
| 2026-06-18 | Uploaded refined V5.5 Slide 3 review copies to Drive | Imported the active V5.5 Slide 3 signal-response PPTX and DOCX as native Google Slides/Docs, verified native MIME types by connector readback, checked the large uploaded Slide 3 thumbnail, and updated the active Drive links in `PROJECT_CONTEXT.md`, `docs/AGENT_START_HERE.md`, and `docs/CURRENT_ARTIFACT_INDEX.md`. |
| 2026-06-18 | Switched website stability audit trace to CSV points | Updated `dashboard/app.py` so the selected-well temperature/phase audit plot uses the committed sampled G10015 CSV profile points as the visible temperature trace and no longer draws the older modeled temperature key-depth curve; kept the guarded stability-screen CSV/map statuses unchanged. |
