...
| Authorized runtime | `dashboard/runtime/`, `doe_jupyter_runtime_pack/` | Source-driven readiness, grouped-well split scaffold, three-dataset runtime, local model-run tracker, and a GitHub-downloadable DOE Jupyter transfer pack implemented | Run the Jupyter pack on the DOE desktop with the real local workbooks, then review row-free graph/table outputs before any public release |
