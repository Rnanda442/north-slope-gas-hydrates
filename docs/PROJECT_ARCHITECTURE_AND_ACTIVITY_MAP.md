...
Use the V5.4 corrected deck and companion as the current presentation
baseline. The older Gmail-style prompt and V5.2 diagrams remain provenance and
visual authority. Treat V5.3 as a flawed intermediate/reference, not the
active mentor deck.
...
|---|---|---|---|
| Public atlas | `dashboard/app.py` | Four-page Processing-style visual redesign implemented with legacy route aliases; Explore North Slope exposes the guarded stability screen, and Analyze Hydrates now includes Public ML Readiness, Schema Coverage & Architecture, Target Registry & Leakage, Presentation Exports, and the synthetic ML visual architecture section. The Schema Coverage tab now shows current public counts, a normalized parameter evidence board, public-now versus OSL-later boundaries, blocked reasons, mentor decisions, the intake validator contract, template downloads, and the approved-data field-role table without exposing approved rows. Presentation Exports now previews/downloads V5.4 corrected slide-ready panels and shows source-visual provenance QA. | Polish visuals and keep public/synthetic data boundary verified during deployment |
...
| Well-log tests | `tests/` | 117 project tests passing after the unified map update | Expand with workbook-derived unit, label, and alignment cases |
...
| W15 | Presentation exports and visual provenance QA | Ready | `data/public_ml_products/source_visual_inventory_2026-06-16.csv`, `docs/SOURCE_VISUAL_INVENTORY_2026-06-16.md`, `dashboard/source_visual_inventory.py`, `tests/test_source_visual_inventory.py`, and Analyze Hydrates > Presentation Exports now preview/download V5.4 corrected slide-ready panels while validating local paths, source status, uncited/AI-looking flags, and guardrails | W6, W10, W14 | Website can supply deck-ready visuals and provenance QA before any slide/Word revision |
...
V5.4 corrected Word/PPT rebuild is now the working mentor-review base and has
been imported to Google Drive as native Docs/Slides files for review. V5.3 is a
flawed intermediate/reference, not the active mentor deck. For the
...
`docs/FULL_PROJECT_ML_WORKFLOW_DIAGRAM.md` as the source for the current V5.4
corrected workflow package. Use the current V5.4 diagram assets for that
purpose: the source-backed context panel, parameter range/behavior panels, the
full complex workflow plate, the equations/unit-gate slide, the complex ML
runtime detail, and the V5.4 contact sheet. The V5.4 PPTX/Word companion are
...
| 2026-06-18 | Unified the website North Slope map stack | Made `Unified North Slope Well + Stability Context Map` the main Regional Map view and the primary guarded stability-screen map. The unified map combines stability status points, USGS hydrate AU outlines, GGD223 `pf_depth_m` controls, DNR units, AKDOT roads, Dalton/Deadhorse roads, TAPS, and public field labels; older geoscience/DGGS/status-only maps are retained as reference views. Exported `unified_north_slope_well_stability_context_map_2026_06_18.png` and documented GitHub-safe versus OSL-only layers plus local PC map findings. |
