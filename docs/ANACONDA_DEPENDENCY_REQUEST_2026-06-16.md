...
models. `tensorflow`/`keras` are for the future ANN path, not for current public
claims.
...
conda create -n north_slope_hydrates -c conda-forge python=3.11 \
  streamlit pandas numpy plotly pyarrow pytest \
  geopandas shapely pyproj pyogrio fiona gdal rasterio rtree scipy matplotlib \
  jupyterlab notebook ipykernel ipywidgets nbformat nbconvert \
  pillow python-pptx python-docx lxml openpyxl xlrd pyxlsb pypdf pymupdf pdfplumber kaleido \
  lasio scikit-learn joblib imbalanced-learn xgboost lightgbm tensorflow keras statsmodels seaborn pyyaml
```
