...
`outputs_runtime/` summaries and shows run status, feature families, exclusion
reasons, dataset/sheet inventory, and the stability-to-ML contract without
committing approved rows or row-level predictions.
...
- The full project test suite passed after the DOE model-run tracker update:
  122 tests passed after adding the three-dataset ML pipeline/header-scan,
  transfer-block, and model-run tracker tests. Update
...
- The same page now includes a `Model Run Tracker` tab that reads ignored local
  runtime summaries in DOE, shows model-run specifics, tracks feature/exclusion
  audits, and keeps stability framed as context, mask, confidence, and caveat
  rather than hydrate proof.
