...
`dashboard/runtime/three_dataset_pipeline.py` with the CLI
...
`curated_dataset3.xlsx` as external tests, detects saturation/occurrence
targets, excludes target-like fields and depth from `X_allowed`, applies
train-only 0-1 scaling, and writes row-level predictions/models only to ignored
runtime folders.
...
- The DOE three-dataset approved-runtime runner is implemented in
  ...
  `01_pipeline/run_three_dataset_ml_pipeline.py`. It trains on dataset 1 and
  scores datasets 2 and 3 separately when an approved target column exists;
  otherwise it writes readiness outputs only.
...
- The full project test suite passed after the source-visual inventory update:
  116 tests passed after adding the three-dataset ML pipeline tests. Update
  this count only after a fresh local pytest run.
...
folder, safely bring back summary artifacts only, and confirm target authority,
...
- 2026-06-16: Added the approved-runtime three-dataset ML pipeline scaffold,
  CLI, runbook, and synthetic Excel tests for training on
